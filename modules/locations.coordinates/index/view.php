...
class Locations_Coordinates_Index_View_LC_HC_MVC extends _HC_MVC
{
	...
	// map
		...
		if( (($latitude == -1) && ($longitude == -1)) ){
			$map
				...
				->add_attr('data-address', esc_attr($address) )
				;
		}
		else {
			$map
	...
	// form
		...
		$inputs = $form->inputs();
		if( $latitude ){
			$inputs['latitude']['help'] = __('Current value', 'locatoraid') . ': ' . $latitude;
		}
		if( $longitude ){
			$inputs['longitude']['help'] = __('Current value', 'locatoraid') . ': ' . $longitude;
		}
